...
# Starts FastAPI backend (prefers backend\.venv if present).
...
try {
  ...
  if ($existingHealth -and $existingHealth.status -eq 'ok') {
    ...
    Write-Host "Backend already running on http://127.0.0.1:$port (storage=$storage). Reusing it." -ForegroundColor Yellow
